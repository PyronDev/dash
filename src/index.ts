...
class realm {
	...
	async setUserPermission(userXUID: number, permission: string) {
		...
		var response: any = await putApi(`/worlds/${this.realmID}/userPermission`, this, { "Content-Type": "application/json" }, JSON.stringify({ "permission": permission, "xuid": userXUID }));
		switch (response.status) {
			case 404:
